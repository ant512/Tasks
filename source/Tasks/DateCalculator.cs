...
namespace Tasks
{
	...
	public class DateCalculator
	{
		...
		/// <summary>
		/// Get a flattened array of the tasks in the current tree array.
		/// </summary>
		/// <remarks>
		/// <para>Each item in the array in an anonymous object consisting of a task
		/// and an array of dependencies.  Dependencies are represented solely by the
		/// task on which the current task is dependent.  Phases (tasks with children)
		/// are not represented in the array.  Dependencies on phases are replaced with
		/// dependencies on all children within the phase.  For example, suppose we have
		/// this:</para>
		/// <para>
		/// +-----+
		/// |  1  |---------+
